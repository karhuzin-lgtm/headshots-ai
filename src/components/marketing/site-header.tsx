      ...
      <div className="relative mx-auto flex h-[3.35rem] max-w-[1400px] items-center justify-between gap-3 px-4 md:h-16 md:px-6 lg:px-10">
        ...
        >
          ...
          <span className="hidden min-[380px]:inline">
            <span className="font-display text-[1.02rem] font-normal tracking-tight">Headshots</span>
          </span>
        </Link>
        ...
        <nav
          ...
          aria-label="Main"
        >
          {MOBILE_LINKS.filter((l) => l.href !== "/#examples").map(({ href, label }) => (
            <Link
              key={href}
              href={href}
        ...
        <div className="flex min-w-0 flex-1 items-center justify-end gap-2 md:flex-initial md:gap-3">
          <span className="hidden text-sm font-medium tracking-tight text-muted-foreground md:inline">
            Launching soon
          </span>
          <button
            type="button"
