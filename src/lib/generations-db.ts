...
export async function updateGenerationStatus(input: {
  ...
  // Row is already done — re-read without modification.
 ...
 * call won the claim (row was paid, pending/failed, and has no tune yet).
 * Concurrent webhook deliveries get null and must not start a second Astria tune.
 * Rows with ASTRIA_STATUS_UNKNOWN error are not claimable — they require manual
 * admin verification to avoid duplicate Astria billing on timeout retries.
 */
 ...
 * count reaches expected_count. Merges directly in the UPDATE SET expression
 * so concurrent Astria callbacks see the live row value rather than a CTE
 * snapshot — eliminates the lost-update race. Returns the updated row plus
 * whether THIS call caused the processing→done transition (ready email gate).
...
export async function appendGenerationOutputs(
  ...
  // If this UPDATE returned a 'done' row, this call caused the transition
  // (WHERE status <> 'done' ensures the row wasn't already done).
